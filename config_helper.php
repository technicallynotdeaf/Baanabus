...
// ---------- Google Calendar cache vault ----------

function gcalCachePath(): string {
    sess();
    $uid = preg_replace('/[^A-Za-z0-9_\-]/', '_', $_SESSION['user_id'] ?? 'default');
    return __DIR__ . "/config/$uid/gcal_cache.enc";
}

function getGcalCache(): array {
    $path = gcalCachePath();
    if (!is_file($path)) return ['synced_at' => null, 'calendar_id' => 'primary', 'events' => []];
    if (empty($_SESSION['DEK'])) throw new Exception('Vault locked');
    $dek   = base64_decode(strtr($_SESSION['DEK'], '-_', '+/'));
    $blob  = json_decode(file_get_contents($path), true);
    $nonce = base64_decode($blob['nonce'] ?? '');
    $ct    = base64_decode($blob['ct']    ?? '');
    if (!$nonce || !$ct) throw new Exception('GcalCache: corrupt file');
    $plain = sodium_crypto_aead_xchacha20poly1305_ietf_decrypt($ct, '', $nonce, $dek);
    if ($plain === false) throw new Exception('GcalCache decrypt failed');
    return json_decode($plain, true) ?? ['synced_at' => null, 'calendar_id' => 'primary', 'events' => []];
}

function saveGcalCache(array $data): void {
    $path = gcalCachePath();
    if (empty($_SESSION['DEK'])) throw new Exception('Vault locked');
    if (!extension_loaded('sodium')) throw new Exception('libsodium missing');
    $dek   = base64_decode(strtr($_SESSION['DEK'], '-_', '+/'));
    ...
    if (file_put_contents($path, json_encode([
        ...
        throw new Exception('Failed to write gcal_cache.enc');
    }
    @chmod($path, 0600);
}
